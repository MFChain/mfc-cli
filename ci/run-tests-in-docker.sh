#!/bin/bash
#
# This script is run inside the Docker container and tests mfc-cli
#
set -e
...
expect /opt/ci/test-mfc-cli.expect

# Start mfc-cli in background for additional JSON-RPC tests
screen -dmS node1 bash -c "dotnet mfc-cli.dll --rpc"
